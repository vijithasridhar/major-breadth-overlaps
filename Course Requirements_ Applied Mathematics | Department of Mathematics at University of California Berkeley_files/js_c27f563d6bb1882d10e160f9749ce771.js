...
;
Drupal.behaviors.math_site = function (context) {
  /**
   * Add a caption below images on node-type-page:
   */
  $('#content-area .field-field-image').each(function() {
    var img = $(this).find('img');
    var capt = img.attr('title');
    if(capt) {
      $(this).append('<div class="caption">' + capt + '</div>');
      var capt_width = img.innerWidth() - 6; //6 = padding used in .caption style
      $(this).find('.caption').css('width', capt_width + 'px');
    }
  });

  /**
   * On main nicemenu nav, add a class to the last main menu that
   *  will allow us to shift it to the left a little so it doesn't
   *  fall outside the bounds of the page.
   */
  $('#block-nice_menus-1 ul.nice-menu-down > li.last > ul', context).addClass('shift-last-menu');


  /**
   * On resize, fix width of rows on events block on front page:
   * (It doesn't do it by itself because slideshow sets fixed widths on rows.)
   */
  $(window).resize(function() {
    var events = $('#block-views-events-block_1');
    var width = events.innerWidth();
    events.find('.views-row').css('width', width + 'px');
  });
...
/**
 * Add feature to the submenu to be able to appears on the left rather
 * than right by first checking to see where the menu should appear.
 */
...
/**
 * Functionality to switch back and forth between table view and list view.
 */
Drupal.behaviors.math_courses = function(context) {
  math_courses_displayed = getCookie('math_courses_disp');

  //note: we're not going to use the context, and will assume that there
  // are only ever going to be one list view and one table view on a page.

  var courses_list = $('.view-courses.view-display-id-list_view');
  var courses_table = $('.view-courses.view-display-id-table_view');

  //do we need to set up a view control and all the description controllers?
  if( !$('.view-control')[0] ) {
    if(math_courses_displayed == 'table') {
      courses_list.before('<div class="view-control"><a href="#">Switch to List Format</a></div>');
    } else {
      courses_list.before('<div class="view-control"><a href="#">Switch to Table Format</a></div>');
    }

    //
    //For Hiding and Viewing Descritpions on Semester Pages:
    //
    /*
    courses_list.find('.course-description')
      .hide()
      .after('<div class="descr-control">View Description +</div>');

    courses_list.find('.descr-control')
      .click(function() {
        var nxt_descr = $(this).prev('.course-description');

        if(nxt_descr.css('display') == 'block') {
          nxt_descr.hide();
          $(this).text('View Description +');
        }
        else {
          nxt_descr.show();
          $(this).html('Hide Description &ndash;');
        }
    ...
    */
    ...
    //
    // For Hiding and Viewing Tabular Format for Courses on Semester Pages:
    //
    $('.view-control a').click(function() {
      if(courses_list.css('display') == 'block') {
        courses_list.hide();
        courses_table.show();
        $(this).text('Switch to List Format');
        setCookie('math_courses_disp', 'table', 100);
      }
      else {
        courses_list.show();
        courses_table.hide();
        $(this).text('Switch to Table Format');
        setCookie('math_courses_disp', 'list', 100);
      }
